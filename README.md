...
✅ Все 8 этапов + 4 фазы Next-Gen завершены. Проект полностью реализован.

## Возможности

- **AI-агент** на основе локальной LLM (3 модели: 7B / 14B / 16B)
- **Оркестрация**: автоматическое планирование и выполнение изменений
- **9 инструментов**: файлы, структура, Git, сборка, поиск по коду, поиск использований
- **6 специализированных промптов** с few-shot примерами
- **Расширение VS Code**: отправка запросов и применение изменений (Ctrl+Shift+A)
...
| Ollama | EmbeddingModel | Модель для эмбеддингов | nomic-embed-text |
| Ollama | ModelByTask:default | Модель по умолчанию | qwen2.5-coder:14b-instruct-q4_K_M |
| Qdrant | Endpoint | URL Qdrant | http://localhost:6333 |
| Qdrant | VectorSize | Размерность вектора | 768 |
...
- **9 инструментов**: read_file, write_file, project_structure, read_documentation, update_documentation, run_shell_command, git_diff, search_codebase, find_usages
- **6 промптов**: orchestrator.txt, code-editor.txt, documentation.txt, refactoring.txt, bugfix.txt, feature.txt
- **Расширение VS Code** в папке vscode-extension/
